...
extension Request {
    ...
    public func validate(_ response: Response) -> ValidationResult {
        if
            ...
            return .invalid("status invalid: \(msg)", value: response)
        }
        ...
        if let jsonMatcher = validation?.json {
            if let json = response.json {
                if case let .invalid(msg, _) = jsonMatcher.validate(json) {
                    return .invalid("json invalid: \(msg)", value: response)
                }
            } else {
                return .invalid("failed to decode JSON object from response", value: response)
            }
        }
